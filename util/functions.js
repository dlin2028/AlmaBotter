...
var findMajor = (function() {
  ...
  return function (message) {
    if(majorsSet.has(message))
      return message;
    else{
      let min = calculateLevDistance(majorsArr[0], message);
      let curr = 0;
      for (let i = 1, len=majorsArr.length; i < len; i++) {
        let d = calculateLevDistance(majorsArr[i], message)
        if(d < min){
          min = d;
          curr = i;
        }
      }
      return majorsArr[curr];
    }
  };
}());
